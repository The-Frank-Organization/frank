## BOOT — initialize s2-core.planner for RUN_ID s2

ROLE: Orchestrator Planner
PHASE: SITREP
AUTHORITY: report-only
DISPATCH_ID: s2-boot-s2-core-planner
...
TO: s2-core.planner
CC: operator
SUBJECT: BOOT — initialize s2-core.planner for RUN_ID s2

You are `s2-core.planner`, the Planner of the single build pair for RUN_ID `s2` — the Slice-2
build sprint in the `frank/` repo (branch `main`, baseline tag `s1-close` = main@f0dcb85).
S2 thickens the ENGINE: recovery phases 0–4, durable FIFO, GC/genesis, and the
owed-item-as-typed-record projection — against the LOCKED m-1 store contract.
...
- YOU DID NOT BUILD S1. Onboard to the S1 code + the s1 sprint docs
  (docs/sprints/2026-07-03-s1-slice-1/, incl. its RECONCILE.md) before any S2 work — fresh
  adversarial eyes are the benefit; m-7 (guide, via operator hand-relay) is the continuity.
...
- Gate model is F2 (normal pair plan-review + conditioned delegated dispatch) — conditions
  verbatim in the ROADMAP; any failed condition, hard trigger, or OUT-item touch escalates
  to master via s2.orchestrator-planner. The MCP live-adapter work stays OUT (deferred on
  operator record; no live testbed).
