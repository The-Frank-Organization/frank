...
- **§6 conductor-internal-provenance bullet** + **§0.e fold-log** present in `master-docs/master/domains/m-1-trust-identity/design/2026-06-28-trust-identity-design.md`, as described: `FROM = ROLE = "system"` (reserved token) for genesis/gc/recovery/derived-outbox; **`system` never accepted from public `submit`** (extends I2 reject-unbound, confusion-resistant D4, D5 residual).
...
- Reconciled against `master-docs/master/domains/m-1-trust-identity/design/2026-06-28-trust-identity-design.md` (§0.e ~:50, §6 provenance bullet ~:148) + `.relays/s2/s2-fidelity-m1/SITREP-implementer-20260704-034158.md` (F-M1-1 source, exists). Greps re-run this seat: enum 5×, observer-selected 9×.
