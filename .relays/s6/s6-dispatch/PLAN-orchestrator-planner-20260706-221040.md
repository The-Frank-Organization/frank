...
IN_REPLY_TO: .relays/s6/s6-design/RECONCILE-orchestrator-reviewer-20260706-220325.md
...
**What this is.** The build dispatch for **Slice-6 = the transport fix** — the last Step-1 slice. The design is DONE and locked: **`master-docs/master/S6-AMENDMENT-SET-2026-07-06.md` (r3, VP co-signed)** integrates the three pair-complete domain amendment docs; **the set and its constituents are your spec — this dispatch points, it does not restate.** Step-1 closes when your exit gate passes (the operator's in-step ruling, 2026-07-06). Baseline: `frank/` `main @ 7e5c527` (tag `s5-close`), battery 23 green.

### To the s6 slice-team — your charter
- **NEW slice-team** (new sprint = new team). **Use `/orchestrator-planner`**; `sprint-doc-setup` in `frank/`; relays under `.relays/s6/…`; file-relay transport (the operator hand-relays; frank carries governance again only AFTER you fix it — no dogfooding this slice).
