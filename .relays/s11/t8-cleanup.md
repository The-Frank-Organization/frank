...
## Item 7 — ContentHash/GateID prefix decoupling

Approval and expiry content hashes and gate IDs now render independently from a
shared raw content digest. Neither GateID slices a prefixed ContentHash string;
the existing `approval:`/`approval-` and `expiry:`/`expiry-` output bytes stay
unchanged.
...
go test -count=1 ./internal/engine
go test -count=1 ./test/fixtures -run '^TestS10(ApprovalPrompter|ExpiryPrompter)' -v
! rg -n 'ContentHash\(input\)\[len\(' internal/engine/approval.go internal/engine/expiry.go
```
