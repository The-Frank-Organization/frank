...
- **(f) co-confirm:** the degraded declaration shape for YOUR rows — `routing_assignments` etc. as top-level carriers with prose-documented columns and a **Step-3 enforcement carry** (no nested-column grammar in s5). Confirm this is acceptable for the routing record your §3:164 "seats write routing records" expects, with column-grain validation arriving with the router.
...
- Sources: `.relays/s5/s5-reconcile-audits/RECONCILE-…-045327` (the four audits' reconciliation, file:line evidence) · the s5-fidelity RECONCILE `…-034602` (the settled answers this refines) · `ARCHITECTURE.md` §J2 + §C4 · `frank/` `main @ 67ee23e` (worktrees verified this pass).
