...
**Locked design:** `docs/sprints/2026-07-06-s6-slice-6/designs/s6-slice-6-design.md` **r3 = main@74f6dbe** (DESIGN_DOC_ID `s6-slice-6-design`; GRILL_LOCK `s6-grill-s6-core` = doc §18; r3 = the external-verdict fold — m-7 R1/R2 + m-1 F-S6-M1-4, directed by `s6-core-design-verdicts` 012809; approving review = the FRESH r3 DESIGN-REVIEW per the content-changing-fold rule). §-references below are design sections. **The design rules; this plan sequences it.**
...
- **THE EXTERNAL VERDICTS — ON RECORD, CARRIED VERBATIM (their texts govern; r2 fold):**
  ...
  **m-7 must-revise, PRE-CONCURRED on faithful fold** (`../master/relays/s6-guide-m7/DESIGN-REVIEW-planner-20260707-012324.md`) — R1(i–iii) + R2 folded at design r3 §7/§0/§16; the plan legs land at T8/T11/T12/T15 below; the pair's r3 DESIGN-REVIEW cites the guide relay + fold hunks (no second guide round on faithful fold).
...
- **Dispatch preconditions:** Implementer PLAN-REVIEW approve on THIS r2 + **the m-1 narrow confirm of the R1 shape** (`s6-fidelity-m1/SITREP-orchestrator-planner-20260707-012810.md`, routed by the orchestrator) on record. No `DISPATCH IMPL` before both.
...
Live-seat designation + every step-exit leg (gate-day); s6-close/merge (never implied by green fixtures). The grill items: RESOLVED (§18). External verdicts: ON RECORD and carried (m-1 F-S6-M1-1..5 · m-2 F-S6-M2-1..4 · m-7 R1/R2 via design r3). Pending before dispatch: the fresh r3 DESIGN-REVIEW approve · the PLAN-REVIEW approve on this r2 · the m-1 narrow confirm of the R1 shape.

*Rev log: r1 (post-PROCEED-TO-PLAN) → r2 (the verdict fold, per the orchestrator directive 012809 + the PLAN-REVIEW must-revise 013004: F-S6-M1-1..5 + F-S6-M2-1..4 verbatim in Global constraints with both route-back lists; M1-4 guard → T8; M2-2 ruled render rule → T3/T9; R1 legs → T11/T12 + the `Cmd` file surface; R2 fifth family → T11/T15; locked design advances to r3 @ main@74f6dbe).*
