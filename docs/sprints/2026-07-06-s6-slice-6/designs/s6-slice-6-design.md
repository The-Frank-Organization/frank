...
- Re-mint completion provenance (m-1 option A ruling, `s6-fidelity-m1/SITREP-implementer-20260707-102208.md`): the 0600 binding row stores only `realized_mint_ref` = latest accepted `seat_mint` pivot relay id, written atomically with credential replacement and never used as activation authority or exposed outside custody.
...
- **R1 — the generation boundary for IN-FLIGHT commands (r3; m-7 must-revise folded verbatim; the code fact: `intake.Cmd` carries no session/generation tag):** force-close kills the channel but not commands already intaken/queued before the pivot — a queued old-session BOOT form would pass B-1.2a after the pivot and become the NEW generation's activation record, defeating the re-mint's intent while formally satisfying the order rule. Fold: **(i)** every command is tagged with its **auth generation** at handler-accept time — the seat's current `seat_mint` pivot ref, ONE persisted `Cmd` field, so recovery replay is byte-identical (the §4 stamp-locus principle); **(ii)** the loop typed-rejects any command whose generation ≠ the seat's current generation — class **`credential-superseded`**, path-free, §11 detail parity; **(iii)** FX-B1g gains the IN-FLIGHT leg (a boot form queued from the old session pre-pivot ⇒ rejected, does NOT activate; the new credential's boot does). A queued non-boot old-session submit was already safe (post-pivot minted-not-active ⇒ `boot-required`; rejected records never activate — m-1 §F.1). The R1 shape refines m-1's generation-boundary carry-forward ("records before a re-mint cannot activate the new generation") to commands STRADDLING it. **m-1 CONFIRMED** (`s6-fidelity-m1/SITREP-implementer-20260707-013244.md`) with four sharpenings carried verbatim — **R1-M1-1..4**: (1) the tag is conductor-stamped at AUTHENTICATED handler-accept, never lane-supplied; content = the committed pivot ref or the bootstrap/genesis generation sentinel, NEVER credential material/hashes/paths/binding bytes; it is intake transport provenance — not an m-1 store field, not accepted-record content, not a lifecycle authority (activation derivation reads ONLY committed pivots + accepted governed records); (2) mismatch = typed TERMINAL disposal before any stale command can append an accepted governed record; for stale NON-submit calls the class surfaces as a typed transport/tool refusal — credential invalidation, NOT lifecycle gating (a minted-not-active seat on the CURRENT credential retains `read`/`project`); (3) the FX-B1g in-flight leg rejects as `credential-superseded` (not `boot-required`) + a negative assertion that the tag is never treated as an activation marker or accepted-record field; (4) route-backs binding: tag persisted into accepted records · credential-as-tag · `active`-from-tag · tag exposed beyond path-free diagnostic metadata (relay-id/class only, only where rejection detail needs it) · stale commands activating · the class lifecycle-gating current-generation read/project — any ⇒ m-1 before implementation.
...
*Rev log: r1 provisional pre-grill → r2 GRILL FOLDED (§18 `s6-grill-s6-core`; six operator rows resolved 2026-07-07) → r3 EXTERNAL-VERDICT FOLD (m-7 R1 §7/§16 + R2 §0/§16 verbatim per the pre-concurred must-revise `s6-guide-m7/DESIGN-REVIEW-planner-20260707-012324`; m-1 F-S6-M1-4 §5; directed by `s6-core-design-verdicts` 012809; fresh pair DESIGN-REVIEW required per the content-changing-fold rule) → r3 completed in the same pass with R1-M1-1..4 per the m-1 confirm `s6-fidelity-m1/SITREP-implementer-20260707-013244` + the orchestrator supplement `…-013623` (no extra round minted).*
