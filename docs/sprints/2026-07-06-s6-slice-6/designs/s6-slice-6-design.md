...
**DESIGN_DOC_ID:** `s6-slice-6-design` · **Rev:** r1 (provisional, PRE-GRILL — §18 is a placeholder until the operator grill folds; no lock, no review-consumed-toward-PLAN before the GRILL_LOCK exists)
...
- I-PH (`Field:Class`, path-free) over every surface incl. the four NEW payload families (roster rows · boot-required per-field detail · lock-refusal diagnostic · hint flags); `bounce.Format` stays the single formatter.
...
- **Durable counter:** each journal segment file begins with a header line `{"segment_header":true,"high_water":N}` (N = counter at segment creation); recovery/writer-construction restores `next = max(all headers, all entry ids)+1`. Ids survive GC of drained segments (every live segment's header carries the mark forward) and rotation and crash. Legacy headerless segments tolerated (max of entries). — GRILL row 3 confirms the shape.
...
- **Re-mint (the ratified audit item; GRILL row 1):** a `seat_mint` for an EXISTING seat = re-mint. PROPOSED mechanics: derived work REPLACES the binding row (fresh credential); the old credential dies at `Resolve` (binding replaced) and any live channel authed on it is force-closed at completion (the per-credential `active` index locates it); generation identity = the committed pivot in commit order (m-1 §F.1 verbatim — the records are the generation history; the binding table stays current-credential-only). `Mint`'s `ErrSeatAlreadyBound` guard moves from "reject remint" to "reject duplicate CONCURRENT mint" semantics inside the derived-work path. Routed to the m-1 fidelity packet (§13.3 credential-lifecycle carry).
...
- **Roster:** `project(view=roster)`, seat-scoped to operator + orchestrator through the existing scoping machinery; rows = {seat · `activation_state` ∈ {minted, active} · `bound_now` · role · minted-at · activation record ref · last-accepted-at} — path-free, no credential material, no socket paths (GRILL row 3 confirms the payload shape). Non-privileged seat ⇒ typed refusal. `project`/`read` NEVER lifecycle-gated.
...
3. **B-2 boot fields** (`charter_loaded`, `dispatch_status` + its named enum): the set's own separately-named additive-MINOR vocabulary impact (m-2 §11.2 verbatim). **Reading surfaced for explicit confirm (GRILL row 4 / completion SITREP):** [VP-W3]'s "exactly seven rows" fences the TRANSPORT row set against the activation-marker's reintroduction; the B-2 boot fields are a distinct named obligation of the same co-signed set. This design proceeds on that reading and does not treat it as silently resolved.
...
| FX-B1a..g | lifecycle/roster/admission/derivation/generation (B1g = [VP-W2]) | fixtures + tables | m-7 B-1.5 |
...
## §18 GRILL_LOCK — placeholder (r1)

`GRILL_REQUIRED: yes`. The operator grill (agenda per the r2 dispatch: re-mint mechanics · F11 replay grain/claim · on-disk commitments {lock artifact, segment header, roster payload} · the §14.3 boot-field reading · anything else surfacing) runs before any lock; the GRILL_LOCK folds here as §18 (the s2 §8 shape) and `DESIGN_LOCK_ID` will reference its `GRILL_LOCK_ID`. Resolved rows (never re-asked): branch A · fallback-never-bounce · precision accepted · derived-only activation, no marker · the seven rows · the codec thesis · every pair-locked A/B/C/D contract.
...
*Rev log: r1 provisional pre-grill (this).*
