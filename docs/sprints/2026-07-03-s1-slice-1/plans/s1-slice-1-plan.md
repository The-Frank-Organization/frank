...
**PLAN_LOCK_ID:** `s1-slice-1-plan` · **DESIGN_LOCK_ID:** `s1-slice-1-design` (r3, main@061882a; pair-approved at r2 `…-152445`, r3 = guide-sanctioned should-fix folds) · **Owner:** s1-core · **Date:** 2026-07-03
...
- [ ] **Step 1: failing capability test** — `capability_test.go`: start a server on a temp Unix socket with a 3-tool set; from a test client: (a) `tools/list` returns exactly `["submit","project","read"]` — no more, no fewer; (b) call `submit`, get a response; (c) **server pushes an unsolicited nudge frame and the client receives it while idle** (D-3 r3 invariant); (d) two sockets = two isolated seats. Run: `go test ./internal/channel/ -run TestCapability -v` — expect FAIL (nothing exists).
- [ ] **Step 2: implement against the official MCP Go SDK.** If (c) is unimplementable over the SDK's socket transport, switch to the D-3 fallback (minimal self-hosted MCP framing: JSON-RPC 2.0 over the socket, `tools/list` + `tools/call` + one server→client `notifications/nudge` method) — the fallback preserves per-seat channels, stamped identity, 3-tool registry, push (all four D-3 invariants). Record which path shipped in the task-close commit message.
- [ ] **Step 3: green + commit** — `go test ./internal/channel/ -v` PASS; `git commit -m "s1 IMPL: channel transport + capability check (SDK|fallback: <which>)"`.
...
- [ ] **Step 1: failing tests** — checksum roundtrip (`Seal`→`Verify` OK; flip one byte ⇒ `Verify` errors); crashpoint no-op without env; `WriteFileAtomic` visible-iff-completed. Run `go test ./internal/... -v` — FAIL.
- [ ] **Step 2: implement the three packages.** The atomic-write core:
...
- [ ] **Step 1: failing tests** — commit-shape (one commit ⇒ exactly one `records/<relay_id>.json` + one INDEX row + one rendered `.md` under `projections/relays/<DISPATCH_ID>/` + a mailbox line per recipient, all checksums valid — the F2 shape); C5 (delete/corrupt INDEX.md, a rendered `.md`, a mailbox ⇒ `RebuildProjections` restores all from canonical; append-only INDEX preserved). Run — FAIL.
- [ ] **Step 2: implement**; INDEX row format mirrors the v2.8.8 columns (REUSE-AS-SPEC'D); rendered `.md` = view-only serialization of the record.
- [ ] **Step 3: green + commit** `s1 IMPL: store pivot + redo projections (F2-shape, C5)`.
...
**Interfaces produced:** derived-intent classes `{park(gate_record_ref), outbox(gate_record_ref), outbox(held_record_ref)}`; `gate.Complete(store) error` — the idempotent scan (missing follow-ups committed, keyed by ref; runs on the loop turn in normal operation, single-threaded pre-open at recovery); ODB item JSON `{item_id, gate_record_ref, seat, gate_category, created_ts, schema_version}` (open envelope — NOT closed, no model_name); park state = store records; wake = verdict commit then one Push; verdict lineage: must parent the parked gate record; one-shot per decision.

- [ ] **Step 1: failing tests** — B4 (one gate-bearing accepted record ⇒ exactly one `outbox/<item>.json` via one pivot; no drain code path exists — grep the binary's symbols/source tree for any network egress in `internal/gate`); W1 (parked lane receives nothing while parked; operator verdict via operator channel ⇒ wake push exactly once; kill -9 while parked ⇒ Phase-restore from records alone); C7 (crash after gate-accept before follow-ups ⇒ recovery yields exactly one park + one outbox item; crash again DURING `gate.Complete` ⇒ still exactly one of each); A4 (two racing verdicts for the same parked decision enqueued concurrently ⇒ exactly one accepted, loser typed-rejected; restart ⇒ still resolved once — D-11, the licensed §2.4 instance); H-r3 (a `held` record from Task 6 ⇒ exactly one operator-visible outbox item; crash between held-commit and derivation ⇒ completed at recovery). Run — FAIL.
- [ ] **Step 2: implement**; **Step 3: green + commit** `s1 IMPL: gate/park/wake/outbox + held visibility (B4, W1, C7, A4, H-r3)`.
