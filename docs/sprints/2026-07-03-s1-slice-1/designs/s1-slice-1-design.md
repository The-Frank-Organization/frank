...
**Status:** DRAFT r1 — awaiting Implementer DESIGN-REVIEW (Template-I request sent per the dispatch)
...
| gate_category | header | agent_enum_pick, monotonic-toward-A | S1 subset of the byte-exact §J2 tokens (exact subset fixed at PLAN with guide visibility); affordance constrained to `[floor, A]`; `other`→A fail-safe |
| gate_category_raised | header | system computed | bool; recorded whenever the floor wins (the ③ guardrail-adjacent portion, D-7) |
| grant | header | seat_scoped_enum | `{dispatch-impl, dispatch-merge}`; **absent from pair-seat forms entirely**; `dispatch-merge` visible only in `PHASE: MERGE-GATE` (m-2 §11.2 — the fill-time analog of the v2.8.8 tokens; the A2 fixture's second leg) |
...
- **Authority-bearing (S1 rule):** a candidate is authority-bearing iff it carries `grant ≠ absent` **or** gate-bears (`HUMAN_GATE_REQUIRED: yes` or `gate_category ∈ A-set`). Aligned with the m-2 :374 derivation shape; the full `record_kind` taxonomy is S3.
...
- A gate-bearing candidate commits as **`accepted`** (ordinary human-gate parking is never `held` — m-2 :376). The loop then commits, as separate single-pivot mutations: a **park-transition record** (lane → `parked_waiting_human`) and an **outbox-enqueue item**.
...
On every start, before any channel opens (**R-3 rebuild-before-open**): (1) staging cleanup — delete torn staging files (never committed, routine); (2) projection rebuild — replay the redo journal idempotently; every committed record's INDEX row / rendered `.md` / mailbox entries exist; canonical wins; INDEX corrections appended, never rewritten; (3) binding-table restore; (4) re-enqueue `intake − outcomes` in arrival order (the F9 property, run whole); (5) wake re-issue for committed-but-undelivered deliveries and parked lanes per records. Only then accept `submit`. **Not built (S1):** genesis validation, canonical-checksum quarantine disposition, GC/segment rotation, the reified phase-0→4 machine.
...
| id | asserts |
|---|---|
| B1 | only committed `accepted` records are served/delivered; a hand-written file dropped into `records/` is not served as a record *(claim scoped: detection is S2 checksum-disposition; S1 asserts the no-tool-path property — D5 stated)* |
...
| G | registry enumeration = exactly `{submit, project, read}` per seat; scripted confused-seat "edit the store file at <path>" turn has no tool to express it; ③ leg: a B-pick over a known-A condition ⇒ raised to A + `gate_category_raised` recorded, never delivered as B |
...
1. MCP Go SDK per-seat-socket transport capability check — first PLAN task; fallback named in D-3.
2. Exact S1 `gate_category` token subset — byte-exact from ARCHITECTURE §J2, fixed at PLAN with guide visibility.
