...
- **Transport:** one **Unix domain socket per seat**, under a conductor-owned runtime dir, carrying MCP (JSON-RPC 2.0). Primary: the official **MCP Go SDK** with a socket-stream transport; **PLAN-time capability check** — if the SDK's server transport cannot ride a per-seat socket cleanly, fallback is a minimal self-hosted MCP framing (the surface is three tools; the protocol subset is small). This check is the plan's first task, not a design unknown that re-architects anything. **Fallback invariants (r2; extended r3 — guide should-fix ②):** whichever framing ships must preserve per-seat channels, channel-stamped identity, the exact `{submit, project, read}` per-seat registry, **and server-initiated nudge push on the held per-seat connection** (m-7 §8.3 — delivery/wake is a push `write()`, not client polling; a framing that drops push breaks L1/W1). The first-task capability check tests for push explicitly.
- **mint_seat (conductor-internal, no agent-facing verb):** operator/spawn tooling asks the conductor to mint; the conductor creates the seat's socket endpoint + an unguessable credential (≥128-bit random), records `seat ↔ credential` in the **persisted binding table** (conductor-owned file, store-adjacent, never a seat surface; written via the same stage→fsync→rename primitive as every conductor file write — r2, reviewer watch item), and provisions the mailbox. Credential + socket path are delivered only to that seat's lane via its private env/fd at spawn (DI-2 realization; **confusion-resistant only — D5:** same-uid processes can enumerate sockets). **Credential lifecycle (r5 — the m-1 :228-229 carry, landed):** S1 defines **no remint/recycle/rotation operation** — exactly **one credential generation per seat** for the life of the S1 store. `mint_seat` on an already-bound seat **rejects with a typed error**: no second credential is generated, the binding table is unchanged, and the existing credential keeps resolving. No code path creates a parallel or stale credential (nothing exists to rotate, so no stale generation can survive); a credential absent from the binding table rejects before anything is staged (the B2 unbound-reject). Rotation/revocation land with m-1's credential-lifecycle work in a later slice — additive on this single-generation base, no re-cut.
...
- A gate-bearing candidate commits as **`accepted`** (ordinary human-gate parking is never `held` — m-2 :376). The loop then commits, as separate single-pivot mutations in the same FIFO turn: a **park-transition record** (lane → `parked_waiting_human`) and an **outbox-enqueue item** — each keyed by `gate_record_ref` (the gate record's `relay_id`), which is the idempotence discriminator *(r5 alignment note: for a gate-sourced item, `gate_record_ref` IS the r4 `source_record_ref` with `source_kind=gate` — one key scheme, two spellings)*.
- **Crash-window healing (r2 — blocker 1):** the committed gate-bearing `accepted` record IS the durable derived-work intent. Recovery (D-9 step 5) scans committed gate-bearing accepted records and idempotently commits any missing park-transition / outbox-enqueue follow-ups (dedupe by `gate_record_ref` — a follow-up that already exists is never re-committed) **before channels open** (R-3 extended to derived-work completion). A crash after gate-accept and before follow-up completion therefore converges to exactly one park record + one outbox item; a re-crash during completion re-converges (idempotent by the same key). Fixture C7. **(r3, guide sharpening (a)):** derived-work completion executes on the single-writer commit path — during normal operation it runs on the commit-loop goroutine in the same FIFO turn; at recovery it runs single-threaded before the loop and channels open — so the sole-store-writer discipline holds during recovery too.
- **Held-visibility (r3 — guide should-fix ①, shape (i) as recommended):** the m-2 carve-out, stated: a `held` record is consumed by gate/escalation/operator-resolution machinery and is **operator-visible by locked text** (m-7 :100 "parked operator-visible"; m-2 :76/:376) — never by downstream work authority. S1 realizes the visibility through the SAME derived-work mechanism: a committed `held` record is a derived-intent class exactly like a gate record — it derives an ODB/outbox item (keyed by the held record's `relay_id`), committed on the loop or completed idempotently at recovery. The §6 self-exclusion rule survives: the fault path that produces `held` passes through no gate that could hold *it*; the outbox derivation is post-commit derived work, not a gate. Held *resolution* flow (re-run / operator verdict producing the next record) remains S2 — S1 surfaces the held item to the operator and never implies live held-resolution (SWEEP covers the wording).
- **ODB item envelope (O-3, open; r4 two-source shape — plan-review blocker):** `{item_id, source_kind: gate | held, source_record_ref (relay_id), seat, gate_category (present when source_kind=gate; null for held whose embedded candidate carries none), created_ts, schema_version}` — exactly one `(source_kind, source_record_ref)` pair per item (mutual exclusion by construction), and that pair IS the derived-work idempotence key for both C7 and the held derivation. The minimal envelope S1 needs; the schema is m-6/m-3-owned and explicitly **not closed** here; **no `model_name` slot is pre-built** (⑤ is the S4-bound §C4 carry, cited per the guide's ledger note).
...
| id | asserts |
|---|---|
| B1 | **(r2 reword — blocker 4; r3 held-carve-out — should-fix ①)** no seat-facing tool/API path can create or serve a record except conductor `submit` (registry enumeration + the scripted confused-seat leg); only committed `accepted` records are served/delivered **to downstream work consumers — with the locked m-2 carve-out beside it: a `held` record is consumed by gate/escalation/operator machinery and is operator-visible (via its derived outbox item, D-7), never by downstream work authority** (m-7 :100; m-2 :76/:376). Direct record-file injection detection/quarantine is S2 checksum-disposition; the S1 direct-file leg is limited to what S1 actually detects (torn/staging files cleaned at recovery, never served). **D5 stated plainly:** a same-uid shell can write a syntactically valid record file S1 will not detect — accepted residual |
...
| H | **(r2 — blocker 3; r3 leg — should-fix ①)** an orchestrator-authority relay carrying no `grant` and no gate field (e.g. `PHASE: PLAN` from an orchestrator-planner-stamped seat) is classified authority-bearing: forced validator-throw on it ⇒ `held` (not `rejected`), and its lineage edges block on failure — the r1 rule would have let it bypass. **r3 leg:** the committed `held` record derives exactly one operator-visible outbox item (crash between held-commit and derivation ⇒ recovery completes it, C7 mechanics); no downstream work consumer ever receives the held record |
...
3. **Advisory sharpening (a):** D-7 now states derived-work completion executes on the single-writer commit path (loop goroutine in normal operation; single-threaded recovery before open).
4. **Advisory sharpening (b)** rides the PLAN, not this doc: S2's owed-item projection generalizes the C7 mechanism — at S2, C7's scan becomes an instance of it, not a second parallel mechanism.

r3 changes NOTHING the r2 approve locked beyond these guide-sanctioned folds; the r2→r3 delta is flagged in the gated PLAN relay for the Implementer's PLAN-REVIEW to check alongside the plan.

## 10. r4 fold-log (s1-core.implementer PLAN-REVIEW `.relays/s1/s1-core-plan-lock/PLAN-REVIEW-implementer-20260703-162202.md`, blocker 2)

One-line envelope fix, no re-architecture: the D-7 ODB item envelope becomes the two-source shape `{item_id, source_kind: gate|held, source_record_ref, seat, gate_category (gate-only; null for held), created_ts, schema_version}` so a held-derived item has a typed origin and the derived-work idempotence key is the same `(source_kind, source_record_ref)` pair for both classes. O-3 openness, no-`model_name`, and every other r3 line unchanged.

## 11. r5 fold-log (m-1 fidelity `s1-fidelity-m1/SITREP-implementer-20260703-171028.md` F-M1-1 + guide nit, routed by `s1-core-plan/SITREP-orchestrator-planner-20260703-171643.md`)

1. **F-M1-1 (m-1 :228-229 carry landed, shape (b) — true to this design, which defines no remint/recycle):** D-3 now states the single-credential-generation invariant — `mint_seat` on a bound seat rejects typed with no second credential and no binding change; no stale/parallel credential can exist; unbound credentials reject before staging; rotation/revocation = later m-1 slice, additive. Plan Task 5 carries the matching E2 fixture legs.
2. **Guide nit:** D-7 crash-window key parenthetical — `gate_record_ref` IS `source_record_ref` with `source_kind=gate`; one key scheme.
Nothing else touched; guide/VP/m-2 approves stand per the orchestrator's routing (the fold touches only the m-1 surface + a doc parenthetical).
