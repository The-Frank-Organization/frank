...
**Status:** r1 — drafted against dispatch r2 (`.relays/s2/s2-core-design/DESIGN-orchestrator-planner-20260704-005310.md`, GRILL_REQUIRED: yes) + the de-provision supplement (`…-005315.md`); operator grill run 2026-07-04, GRILL_LOCK at §8 (`s2-grill-s2-core`); awaiting s2-core.implementer DESIGN-REVIEW
...
- **No-authority-until-open as STRUCTURE, not call order:** `recover.Run(root, cfg) (*engine.Ready, error)` — `engine.Loop` start and `channel.ServeAuthenticated` **require the `Ready` value** (unexported-field struct only the phase machine constructs). Code cannot open channels or consume authority without a completed phase 4. During phases 1–3, reads of committed records may be served (m-7 :95); zero mutation, zero authority consumption.
...
- **Genesis record** = the first canonical record: `{schema_version, config_digest, address_space_seed, created_ts}` — committed via the standard pivot by a new `store.Init(root, pinnedConfig)`; fixed `relay_id` (**proposal: `genesis`**, §4.3). **Idempotence** = the existing duplicate-relay-id rejection (store.go:67-71): re-init against an existing genesis is a typed no-op refusal — nothing written, fixture-proven (the "genesis idempotent" gate line, pinned as this property).
- **The pinned artifact set at S2** (Q1=(a)): `{fieldspec/registry.json, engine.json}` — `engine.json` is new: `{gc_enabled: false, segment_rotate_bytes: 4194304}` (**grill row 2 defaults**). **Deterministic digest input** (sharpening 3): SHA-256 over a canonical manifest — members sorted by name, each entry `name '\0' sha256(bytes) '\n'` — so the S3 change-record can extend the member set unambiguously.
...
- **The LIVE path is swept** (the audits' F-1 bite): after phase 1, all in-store records are verified; post-open operations run on the phase-3 runtime tables (D-6), so the per-operation full-store rescans that today propagate a mismatch as a brick (store.go:114-117 → lineage.go:51-53, derived.go, recover.go) disappear structurally. A mismatch detected on a live `read()` (bit-rot after open) returns a typed `checksum-mismatch` error to the caller (I-PH-clean) and enqueues an internal quarantine-disposition command into the FIFO — the disposition itself executes on the loop (single-writer discipline; no reader ever mutates).
...
- **The mechanism:** an obligation class = `{source predicate over committed records, completion key, completer}`; **`open(class) = source-record with no completion-record`**, computed at phase 3, maintained on the loop. Instances:
...
- **Mutation classes swept at S2:** the six S1 classes + `{genesis, quarantine-disposition, gc-marker}`; owed-item and disposition commits are ordinary submit-accept instances (listed, swept under that class). **F9 and F11 re-run whole** under the phase machine (gate line G5); the S1 suite runs unmodified as the regression floor (G6).
...
| S2-V3 | genesis missing/digest mismatch ⇒ diagnostics mode: `submit` absent from every rendered registry, `project`/`read` alive, zero store mutation, operator diagnostics report present + path-free (I-PH grep) | G1/G3 |
...
| S2-PM2 | structural no-authority-before-open: channels/loop unconstructible without the phase-4 `Ready`; compile-level + a runtime probe during phases 1–3 (submit refused, reads served) | G1 |
...
## 4. m-1 fidelity proposal surface (self-contained; every shape is a PROPOSAL — m-1 fixes it)

1. **`record_kind: owed_item`** header + payload fields `{owner, source, target_surface, disposition_path}` (from the s1 ledger's typed shape); envelope compliance unchanged (checksum, schema_version, intake_id when loop-committed).
2. **Disposition record** — header `record_kind: owed_disposition` + `disposes_owed: <relay_id>` (mirrors the S1 `parks_gate`/`resolves_gate` idiom); one disposition per owed id (duplicate ⇒ typed reject).
3. **Genesis record** — fixed `relay_id: genesis`, fields per m-7 :136; lives in `records/` as record #1 (layout call = m-1's).
4. **Incident record** — HELD-class, references quarantined `relay_id` + failure class; compound single-pivot.
5. **`quarantine/`** — new store-root member (locked location m-7 :91); eviction = rename of the corrupt file, name-preserving.
6. **GC-marker record** — body names collected segments (canonical-sufficient); one per GC pass.
7. **Journal segmentation layout** — `journal/intake/<seq>.jsonl`, `journal/redo/<seq>.jsonl`, monotonic zero-padded `<seq>`, highest = active; rotation at configured bytes.
8. **Derived-record body embedding** (canonical-sufficiency, D-6) — outbox/derived records embed their item payload in body; no envelope/layout change, flagged for fidelity awareness.
9. **`projections/owed/OPEN.md`** — derived, rebuildable open-set artifact (layout home = m-1's).
10. **Pinned-config manifest + `engine.json`** — store-adjacent conductor-owned config members whose digest genesis pins (Q1=(a) scope; the artifact is operator-ratified build config in the m-2-locked shape, no domain-author stamps).
...
## 9. Claim-sweep note (m-7 §16 classes, run over this doc)
