...
1. **Phase scope = Step-3 DESIGN — the barely-enough-coding-agent MVP on the governed courier, per the RATIFIED architecture reframe AS AMENDED by the RATIFIED MVP amendment.** The architecture-of-record is **`master/STEP-3-ARCH-AMENDMENT.md`** (operator-ratified 2026-07-15 at SHA-256 `2d240eb6…`, VP-approved `step3-arch-packet/063000`) **as amended by `master/STEP-3-MVP-AMENDMENT.md` r7 (operator-ratified 2026-07-16 at SHA-256 `2f75f2a1…`, VP byte-bound approve `…-035505`; supersedes exactly four named packet fragments; the packet file stays byte-exact)**: **the conductor is ONE isolated governed-relay service, NOT the app's central hub**; the app shell around it = **m-10 App Control Plane/Supervisor** (a MODULE in the app main process — grill #1) + **m-8** connector (a separate app-side process; holds provider creds + does the wire) + **m-9** worker (the supervised worker process acting FOR the logical m-9 seat — the seat credential is m-7-broker-held, F60/F66) + human/terminal surfaces; provider traffic bypasses the conductor. **Step-3 = the MVP** — a coding-agent worker with local tools `read/write/edit/apply_patch/bash` + the conductor as a NATIVE relay tool (`relay.submit/project/read`; MCP retained for foreign harnesses), one honest governed turn, live E3, a single pinned **app-side run manifest** carrying the operator-ratified **8-name tool-dispatch constant** with F59 one-shot-ticket enforcement (the entire permission/authority system → Step-4; NO routing execution; m-4 routing + benchmark/spawn/steer defer to Step-4). The five original Step-3 DESIGN/amendment lanes were **HELD + re-cut**; **the first stage = the amendment's §7 acyclic graph** (stage-1: m-2 form→tool-schema mapping · m-7 transport + channel/broker · m-10 app-IPC/manifest seam · m-1 secret-boundary + seat identity · m-3 egress/E0/E3-evaluator → stage-2 m-8 → the m-9↔m-10 lifecycle halves → the m-9/m-10 designs → the Master+VP interface-lock; the m-5 ceiling-host amendment STOOD DOWN — `643dd7c2…` is the Step-4 basis). The reframed kickoff sections are **SUPERSEDED** (old kickoff hash `983508fc…` preserved as the historical lock). **Step-2 is CLOSED** (2026-07-14,
   tag `s11-close` at `main@502e06c`, pushed frank-dev; the step-exit test PASSED — three legs live on the dogfood
   store + the INV-CATALOG red-battery demo + uncached green; **operator-ratified 2026-07-14 in-session**; VP
   adversarial close-confirm filed `step2-step-exit/RECONCILE-orchestrator-reviewer-20260714-211500`). The gates are now
   unfakeable (observe-as-send + evidence ladder), lanes self-pace (park/wake + buckets), and the constitution is ten
   executable INV-CATALOG laws. **Step-1 CLOSED 2026-07-08** (`s6-close`); **Steps 1+2 = the trusted courier is real.**
   **The OPERATING MODEL SHIFTS at Step-3 (operator-decided 2026-07-14): the team now RUNS ON frank as courier** (relays
   flow through `submit`/`project`/`read`, not operator hand-relay), with an **independent slice orchestrator team per
   slice/bundle** owning **local detail-design + plan + impl** against the **m-x-authored spec-of-record** (the m-x pairs
   are the PMs), **escalating UP through master to the m-x planners** on (a) a spec mistake, (b) a better way, or the
   standing `DELEGATED_DISPATCH_AUTHORITY` triggers — the deferred **T4 tier** (`master-org-decomp`) now activated because
   frank automates the relaying. **First Step-3 acts (the bridge): the operating-model spec + the frank live relaunch +
   shakedown** — build slices open on frank after. **T5/T10 rescoped as Step-3 carries** (behind g2/dc). **Design-lock
   stays terminal for DESIGN** (changes go through review-driven *amendment*, never silent re-design); **MERGE stays gated**
   per the roadmap step-exit tests.
2. **Governance docs + relays live in cwd (`master/`); build CODE (now Step-3) goes in `frank/`** (the build repo — **formerly `pcode/`**, renamed 2026-07-03). `frank/` is the target git repo — **now the build target** (no longer *future*). The design-of-record (`ARCHITECTURE.md`, the domain docs), the relay trail, and the dashboard stay in cwd; the conductor's implementation lands in `frank/`. Don't put governance docs in `frank/`, or code in cwd. *(Historical relays / RECONCILE entries that say `pcode/` are the append-only record from before the rename — read them as `frank/`.)*
...
| **Sr EM / Tech Fellow ×7** (Conductor) | each a pair: design-lead `.planner` + adversarial design-reviewer `.implementer` | `m-1 … m-7` |
| **Sr EM / Tech Fellow ×3** (Harness Runtime — Division II, **STOOD UP Step-3**; greenfield, DESIGN-only until their domains lock; boundaries re-cut by the ratified reframe `master/STEP-3-ARCH-AMENDMENT.md`) | each a pair: `.planner` + `.implementer` | `m-8`, `m-9`, `m-10` |
...
| **m-8** | **Provider Adapters / connector** (Harness Runtime — Division II, **Step-3 reframe: app-side connector**) | provider wire translation · normalized provider events · the **factual lane catalog** (`{model_id, provider_id, serving_profile_id, compat_mode}` + spec-sheet) · conformance fixtures · **(reframe) HOLDS provider credentials at runtime (m-1 boundary) + OWNS the connector-credential contract + is the last pre-wire enforcement host (`freeze→authorize→attach→send`, app-side)**. **Does NOT own** egress *policy* (m-3) · routing judgment (m-4) · authority enforcement (m-5/m-10). **NOT a conductor seat.** | m-1 secret-boundary review · m-3 provider-send policy · m-10 supervision + opaque credential-reference · m-4 policy overlay (Step-4) · m-9 (the runtime that calls it) |
| **m-9** | **Model Runtime** (Harness Runtime — Division II, **Step-3, greenfield/DESIGN-only**) | the model-turn / session / context state machine · governed *requests* to tool execution (a parsed tool call stays INERT until the trusted authority/tool-exposure path authorizes it) | m-8 adapters + catalog · m-7 (does NOT re-own its loop/recovery/trusted-config/seat-guardrail) · m-5 authority ceiling (does NOT re-own) · m-3 observe |
| **m-10** | **App Control Plane / Supervisor** (Harness Runtime — Division II, **NEW at the Step-3 reframe; greenfield/DESIGN-only**) | the app-shell hub: worker lifecycle+supervision · app-side IPC+backpressure · the **run manifest** + run/session-state persistence · the app scheduler + active-turn lease + `turn_epoch` fencing · the app-side authority **ENFORCEMENT point** (MVP: hosts the operator-ratified fixed tool-dispatch constant + authors the F59 one-shot-ticket protocol; m-5's ceiling plugs into the same seam at Step-4 — m-10 never owns the policy) · connector supervision + **opaque** credential-reference orchestration (never secret bytes, incl. the seat credential). **Owns NO policy; NOT a conductor seat.** *(Runs as a MODULE in the app main process per the ratified MVP amendment §2b.)* | m-5 ceiling interface (host only) · m-9 worker lifecycle · m-8 connector supervision · the conductor (only via the m-9 worker seat's verbs) |
|  | *Cross-domain integration + protocol/governance evolution + collision arbitration for the runtime seams stay with the **CTO + VP**. Division II boundaries (conductor vs app shell; m-8/m-9/m-10 seams; provider egress app-side; m-5 ceiling-host; the E0 carrier; the operator direct route) are **re-cut by the ratified `master/STEP-3-ARCH-AMENDMENT.md`** — which SUPERSEDES the conductor-hosted framing in `STEP-3-KICKOFF.md` **§§1–3 AND §§5–8** (V2/V3→Step-4, the amendment set re-cut, the spine-non-terminal framing re-cut; V1/E3/T4-gate survive).* | | |
...
- **Battle reports → frank's hardening backlog** (the dogfood friction/failures + external audits that harden frank from a
  courier into the full governed dev-team *ecosystem*; the team's friction IS the product roadmap — operator 2026-07-15):
  **`master/FRANK-HARDENING-BACKLOG.md`** — append each battle report + its mechanization findings; carries the two adopted
  rules (T4-preflight-before-code-token; gates-cannot-check-queue-before-m-x-on-frank).
