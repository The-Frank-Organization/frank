...
- `python3 ~/.codex/skills/tools/relay-lint.py master/relays/c6-differential/RECONCILE-orchestrator-planner-20260702-225941.md` - OK.
...
- `rg -n -e 'deviated_observed := declared_bucket' -e 'bucket_binding_observed := chosen_model' -e 'egress_scan_result=blocked' -e 'egress_blocked' -e 'observe hook is inert' -e 'deviation_reason_code.*config-sourced' -e 'routing_unavailable' -e 'GRILL_REQUIRED FieldSpec' -e 'one compound canonical record' ...` - confirmed the corrected anchors are present.
