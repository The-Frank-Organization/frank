...
- m-4 Routing & Policy — implementer `approve` rev1 (`c2-design-m-4/DESIGN-REVIEW-implementer-20260629-203329.md`); doc `master/domains/m-4-routing-policy/design/2026-06-29-routing-policy-design.md`.
- m-3 Observation & Evidence — implementer `approve` (`c2-design-m-3/DESIGN-REVIEW-implementer-20260629-211003.md`); doc `master/domains/m-3-observation-evidence/design/2026-06-29-observe-evidence-design.md`.
...
- (a) define the **1–3 Step-1 routing-template structures + lineup** (the GL-4 scope addition — seats/panes/gate-set/read-only-ness);
...
m-5's **full** archetype design stays **c3**; this is only the Step-1 slice. This **satisfies your lock prerequisite** (the "narrow m-5 consumer review on the draft designs" option) AND realizes GL-4. **Confirm:** the scope is bounded correctly and preserves m-5 ownership of the concrete tag-space + invariants.
