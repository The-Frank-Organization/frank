...
| m-6-F6 | M | Meeting-lane routing keys on GRILL_REQUIRED as a 'LOCKED field', but m | master/domains/m-6-human-surface-scheduler/design/20… | Have m-2 declare GRILL_REQUIRED's registry row (owner/type/values — presumably the ported upstream-protocol header, agent-raised… |
...
- **GRILL_REQUIRED field status (m-6-F6):** reconcile the meeting-lane routing that keys on `GRILL_REQUIRED` as a 'LOCKED field' against m-2's schema status of that field — align to m-2's actual FieldSpec.
