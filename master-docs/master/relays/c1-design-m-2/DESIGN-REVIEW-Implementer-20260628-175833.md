...
Review asks:
1. The two-state `submitted` -> `accepted` model and the named lineage-blocking record classes restore the upstream protocol's "blocks before dispatch" guarantee for authority-bearing records. The key invariant is consumer-side: only `accepted` records are deliverable/consumable.
