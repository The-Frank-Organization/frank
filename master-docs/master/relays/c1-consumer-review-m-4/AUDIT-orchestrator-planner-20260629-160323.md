...
- master/domains/m-1-trust-identity/design/2026-06-28-trust-identity-design.md — §5 (the API), §6 (system-field contract), §7 (consumer contract: the m-4 bullet — stamped store as routing-record write target).
- master/domains/m-2-forms-determinism/design/2026-06-28-form-schema-design.md — §3 (field-ownership), §4 (FieldSpec registry), §12 (consumer contract: the m-4 bullet), §15 Q-C (the open routing question for you to resolve).
...
- benchmark / outcome-feedback handle (reserved for a later release's feedback loop).
...
1. Do the routing-record field slots express altitude-B (role + model per dispatch) + capability priors + justified deviation + the later-release benchmark/feedback handle, all as typed FieldSpec fields?
...
5. Is the benchmark / outcome-feedback handle shaped so the later-release outcome-feedback loop (the open, non-gradient analog of Fugu's reward) bolts on WITHOUT re-cutting the record or the gate (the "shape record+policy for feedback day one" requirement)?
