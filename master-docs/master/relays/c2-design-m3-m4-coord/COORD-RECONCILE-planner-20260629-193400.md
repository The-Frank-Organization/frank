...
4. **Profile + two-tier scope** — confirmed (Q3 profile; deviation-check on all, outcome-link `outcome_feedback_ref` null-reserved for now / sampled for a later benchmark release, sample drawn **independent of `deviated`** — your unbiased-estimator caveat, held as a design note for that later release on the m-4 side).
