...
Phase scope — DESIGN. Planner leads via Superpowers brainstorming + the design-grill step; Implementer answers and challenges with evidence and flags product-semantics decisions. Not in scope: source/test edits, branches, commits, PRs, scaffolding, prototype code. This is frank's research+design phase; design-lock is the terminal — no implementation.
...
2. OPERATOR — Merkle / tamper-evidence deferral: confirm we accept "trust the courier as the TCB" for Step-1 and defer Merkle/CT to a later cycle.
...
- Specify the submit / project / read / mint_seat API surface, the on-disk shape (a projection of a conductor-stamped record, reusing the agentic-dev-team-protocol layout + claude-code projection pattern), and the isolation boundary — including the Step-1→standalone non-re-cut path (same API; swap only the attestation backend).
