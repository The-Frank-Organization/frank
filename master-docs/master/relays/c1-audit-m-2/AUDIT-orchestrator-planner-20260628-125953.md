...
- Upstream current state: the upstream protocol release corpus (not vendored) — <upstream relay-lint tools>/relay-lint.py (the ~1448-line linter, the prose-ambiguity defenses to be dissolved) and protocol.md (the ~50-field header schema across its field clusters), plus the three locked field-ownership decisions described in the pre-build design-state export (not vendored).
...
- Export design intent: the pre-build design-state export (not vendored) — the structured-form / determinism model, the "canonical iff a mechanical consumer reads it" rule, the three field-ownership locks, and the CodeAct "forms beat code for the control plane" reasoning.
...
1. A 4-bucket verdict on the frank form schema + field-ownership model vs what the upstream protocol's linter and protocol header already encode.
2. A map of which upstream prose-ambiguity defenses (strip_fenced/inline, ambiguous-continuation, detached-row, duplicate-block, bare unfenced DISPATCH-token machinery) become moot once the envelope is typed, and which lint logic survives as form-validation plus a lineage engine.
...
- Reads: the upstream linter + protocol schema; agent-scripts JSON contracts; jcode/claude-code message typing; the export form model.
...
0. Duplicate/already-built check: the upstream protocol header and relay-lint ALREADY encode ~50 fields plus field grammar — audit how much is promote-and-formalize vs net-new, and do not rebuild what survives.
1. If a typed envelope cannot preserve an upstream gate (for example a lineage check), name the gate and route the dependency to the orchestrator.
...
Deliverable:
A file-relay AUDIT report from each seat independently, then a reconciled view: the 4-bucket verdict; the dissolve-vs-survive map of the upstream lint logic; the form-schema design recommendation; the consumer boundary contract; evidence levels (E1 source cites); operator-judgment items or none. No source changes, no PR. Include ACTIONS_GIT_REF and FINAL_GIT_STATUS_SHORT (read-only phase; cwd is not a git repo, so the structured unavailable form is expected).
