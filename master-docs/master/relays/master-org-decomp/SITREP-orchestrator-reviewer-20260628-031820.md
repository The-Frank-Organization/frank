...
Finding 1 -- product-scope expansion is not locked. The charter's product statement says frank is "the conductor": a thin trusted courier with locked routing/policy/seat-trust decisions, and points to the pre-build design-state export (not vendored). The reviewed relay says frank has expanded into "a model/provider-agnostic, governed, multi-agent dev-team harness shipped as a TUI app" with a Claude-Code-style runtime, Zellij-style multiplexer, and integrated email client. That may be the right destination, but it is a major product-boundary change from conductor-governance substrate to full runtime/product. This is an operator-judgment category: product semantics, broad scope expansion, and ownership. Do not staff Runtime/Product/TUI/Email domains as equal Cycle-1 design domains until the operator explicitly confirms whether Cycle c1 designs (A) conductor substrate only, (B) conductor plus runtime seams, or (C) the full TUI/runtime/email product.

Finding 2 -- flat-and-lean Cycle 1 is the right operating shape, conditional on that scope lock. I agree with rejecting the 4-tier recursive org as the current operating structure. Evidence: the charter says pairs are independent operator-relayed sessions, the current phase is AUDIT + DESIGN only, and the upstream protocol models one orchestrator pair over flat downstream pairs. Adding T2 division orchestrators and T4 feature teams now multiplies manual relay hops and invents nested-run conventions before frank exists to automate them. Capture recursive/fractal org support as a frank design requirement, not as this phase's manual operating topology.
