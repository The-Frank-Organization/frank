...
1. OPERATOR — **executable-claim execution surface**: registry-approved check descriptors vs arbitrary project commands. Recommend Step-1 ships observe-as-send on passive conductor-derived E1/E2, the conductor-EXECUTES mechanism a reserved seam operator-gated at activation. Grill: confirm Step-1 ships without arbitrary execution; descriptors are the eventual form.
...
- **F5 acceptance criterion (no overclaim):** state novelty precisely. The agentic dev team protocol (E-ladder vocabulary), jcode (permission/review queue), claude-code (Pre/PostTool/Stop hooks), agent-scripts (ODB / egress rules) already provide PARTIAL hook/evidence/egress primitives — the design must PROMOTE those and claim novelty ONLY for the integration (outside-the-lane observe-AS-send + `evidence_integrity` + the unbypassable chokepoint), not for hooks/evidence/egress in general.
