...
3. frank design seed: the exported pillar locks altitude B, a 3-staged policy, and benchmark feedback as a later-release hook (the pre-build design-state export (not vendored) — the adaptive-routing pillar, §16-19). It later resolves model identity as non-trust-bearing payload/bookkeeping and names router as a conductor function (same source, §33-35).

4. the stock protocol gap: the released package is a coordination protocol, not an orchestration runtime (the upstream protocol release corpus (not vendored) — `README.md:1-5`). It explicitly says roles are contracts, not model identities (`README.md:96-100`, `README.md:139-146`). Its route-like fields are relay `TO`/`CC`, lineage, and dispatch authority, not role+model routing.
...
- `benchmark_feedback_handle`: reserved id for outcome feedback in a later release
...
3. Benchmark/outcome feedback in a later release consumes m-3 observed evidence and updates/prioritizes future priors; it is a forward hook now, not Step-0 closure.
