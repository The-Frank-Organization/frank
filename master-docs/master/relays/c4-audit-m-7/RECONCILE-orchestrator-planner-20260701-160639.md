...
- `PRIMARY_BUCKET: still-open` — the entire §2A substrate is net-new; **no existing conductor to promote** (both ran the duplicate/already-built gate: the upstream protocol is "a coordination protocol, not an orchestration runtime").
- Same **promote-PARTS donors** consumed inside the net-new engine: the upstream protocol/lint layer; jcode connection-binding + tmp→fsync→rename write discipline; claude-code per-recipient mailbox + lockfile + re-read-after-lock; codex single-owner serialized writer + ToolExposure split + config-lock replay; external SQLite super-journal / `rename(2)` / `fsync(2)` / Maildir.
...
- `python3 ~/.claude/skills/tools/relay-lint.py master/relays/c4-audit-m-7/RECONCILE-orchestrator-planner-20260701-160639.md` — OK
