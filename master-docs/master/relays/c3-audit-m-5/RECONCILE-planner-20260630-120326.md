...
- **Reject-gates (both):** no value lock in AUDIT; **no m-2 `required_when`/`visible_when` over concrete tag values in c3** (C2.4 reserved opaque atoms only — no m-2 micro-fold, `ARCHITECTURE.md:189-194`); no sensor→actuator in-place upgrade; no conductor/N-pair template at Step-1; narrow any non-Step-1-enforceable invariant to a recorded-contract + later-runtime dependency. Converged.
