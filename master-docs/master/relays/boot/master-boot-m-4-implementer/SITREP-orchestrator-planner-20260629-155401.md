## BOOT — initialize m-4.implementer for RUN_ID master

ROLE: Orchestrator Planner
PHASE: SITREP
AUTHORITY: report-only
DISPATCH_ID: master-boot-m-4-implementer
...
TO: m-4.implementer
CC: operator
SUBJECT: BOOT — initialize m-4.implementer for RUN_ID master

You are m-4.implementer for RUN_ID master — adversarial design-reviewer (Implementer) on the m-4 Routing & Policy domain. In this AUDIT + DESIGN-only phase your job is independent review, not implementation. THIS cycle (c1) your task is the CONSUMER-LENS review of the m-1/m-2 foundational interface sketches from the m-4 perspective — run it INDEPENDENTLY of your planner, then reconcile.
...
Your pair partner (design-lead) is m-4.planner.
Context: m-1 (Trust & Identity) and m-2 (Forms & Determinism) are design-complete and pair-approved; HELD for a joint co-foundational lock that requires your consumer review first. Your consumer fields to validate: the routing-record fields the m-2 schema must express (role+model-per-dispatch, capability-priors, justified-deviation, a DESIGN_RECORD_KIND-shaped record-kind), plus the m-1 stamped store as the write target for routing records (the record's FROM must be the router seat's stamped identity).
