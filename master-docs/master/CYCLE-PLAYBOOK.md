...
`frank/`. We stood the team up as a **standing governing team** (not a transient sprint), modeled as a software
...
- **Lint every substantive relay before handoff:**
  `python3 ~/.claude/skills/tools/relay-lint.py <file>` (exit 0 = OK). Scoped lineage check:
...
- **Toolchain path moved.** The operator changed workstations (Linux → macOS) mid-engagement; the linter is at
  `~/.claude/skills/tools/relay-lint.py` (with a `.codex` mirror). Don't hardcode a home path in docs.
