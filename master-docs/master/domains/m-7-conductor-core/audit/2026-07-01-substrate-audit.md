...
**Provenance / method (CF-2 compliance):** planner-read governing corpus + all six locked m-1..m-6 design docs (contract inputs, read in full); live-instance probe (E2, planner-run); five read-only corpus-lens subagents for the planner's OWN sweep only (the upstream release, jcode, claude-code, codex [operator-directed addition], external web prior-art) — lenses did not simulate, replace, or proxy-author the independent m-7.implementer audit (F4 stands). Load-bearing lens cites spot-checked against source by the planner (hook non-blocking header; jcode `write_bytes_inner`; jcode wire `from_session`). CF-1: full `master/...` paths used throughout.

---

## 0. BLUF

1. **The substrate is net-new — nothing anywhere in the corpus runs a serialized, crash-atomic, guarded relay store.** The upstream release is verbatim "a coordination protocol, not an orchestration runtime" (its README:7): a message format + a truth-agnostic linter, **zero process**. The LIVE `master/` team is the proof by demonstration: 261 INDEX rows with out-of-order and duplicate timestamps from unserialized seat appends, no lock, no daemon, seats (including this author) holding raw store paths.
...
The standing team's own store (`master/relays/`) is the running upstream-protocol instance, and it demonstrates the absence of every §2A mechanism:
...
| any conductor process | **absent** | `ps` probe: no daemon; delivery = operator hand-relay (charter, `master-docs/CLAUDE.md`) |
...
### 3.1 The upstream release (lens: sweep-v288; E1 lens-collected, hook header planner-verified)
Corpus: the upstream protocol release corpus (not vendored). The entire executable substrate is `tools/relay-lint.py` (1449 lines) + a 35-line PostToolUse hook.
...
| **store genesis + GC/retention** | the upstream store layout (promote) | **genesis record (config hash, schema_version) + retention policy — wholly absent** |
...
- **Local-outbox-only:** no seat has an external-send tool (m-5 locked: no `external_send` axis in the initial release); the only egress is the conductor's outbox at the m-3 §7 egress gate (dormant until the m-6 away bridge). m-7 hosts the chokepoint; net-new.
...
|---|---|---|---|---|
| S1 | store append + channel-stamped FROM (m-1) | `master/domains/m-1-trust-identity/design/2026-06-28-trust-identity-design.md` §5 (submit/project/read/mint_seat, :97-107), §6 (on-disk + system fields), §4 (DI-1..DI-5) | run the four verbs; stamp FROM/ROLE from the channel binding; one atomic accepted-append + INDEX + mailbox deliver inside the commit loop; terminal `rejected` on fail; serve `read` to the lineage engine | no — but binding-table persistence across restart is CQ-6 |
| S2 | away-token mint/verify, Seam C (m-1) | m-1 doc §C3.4 refs + `master/domains/m-6-human-surface-scheduler/design/2026-06-30-human-surface-scheduler-design.md` §4 (:85-88), §12 carries | execute `verify()`'s five checks + **nonce-burn as an atomic conductor append in the same critical section as submit** (the double-accept kill, §4.1); sibling-burn per DESIGN-REVIEW §2C | sibling-burn decision-scoped resolution named in §2C but not yet in locked m-1 text — CQ-6 |
| S3 | form render + fill-time authority (m-2) | `master/domains/m-2-forms-determinism/design/2026-06-28-form-schema-design.md` §3 (render semantics), §4 steps 1–6 (:68-74) | render the form trusted-side per (seat, phase, tier, slot): forbidden options **absent**; validate constrained picks (set-membership / monotonic ≥ floor); courier-fill system fields | no |
...
| S10 | routing record + route_dispatch fail-closed (m-4) | `master/domains/m-4-routing-policy/design/2026-06-29-routing-policy-design.md` §3 (:144-163), §5, §7 | sequence routing-record acceptance through submit like any relay; host `route_dispatch()`'s fail-closed refusal; never a silent default model | no |
| S11 | template spawn + pane-spawn (m-4/m-5 contracts; conductor-core named executor) | m-4 §0 GL-4 + §7 (:236-263); `master/domains/m-5-workflows-archetypes/design/2026-06-30-archetype-system-design.md` §7 (:116-127), §10 | open/name panes via existing tmux/zellij/OS-terminal (Step-1-consistent); deliver boot relays; record per-assignment `seat_archetype`+`authority_ceiling` at spawn (F2 home is m-4's record) | mild — pane-spawn mechanics are Step-1 build detail; no contract gap |
...
still-open: the ENTIRE §2A substrate — serialized commit loop, crash-atomic multi-file commit + recovery +
  ...
  (the upstream release: a linter, zero process — verified; live master/: unserialized seat appends — E2; jcode: multi-task
...
already-closed: none — no existing conductor exists to promote or enable. (Distinct from PROMOTE-parts, which
  are components consumed INSIDE the net-new engine: the upstream semantic contract layer via the locked m-2
...
- **Narrow gate:** if DESIGN discovers the multi-file commit cannot keep INDEX.md human-append-readable AND crash-atomic, the fallback (derived INDEX, §7.2) changes the on-disk contract m-1 locked (§6 "reuse the upstream layout") — that would be a targeted m-1 COORD, not a silent change.

## 9. Questions for m-7.implementer (pair reconcile) + operator

- **To m-7.implementer:** (a) does your independent sweep confirm the absence claims (especially: no serialization anywhere in the upstream release / jcode / claude-code write paths)? (b) do you concur the licensed by-construction claim is scoped exactly to §4.1's control-flow property? (c) any hosted contract I mis-bucketed as under-specified vs consumable-as-is? (d) over-reach check: do any of my §7 design questions silently re-own policy?
...
*(Read-only audit; no source / build-repo writes. Relay of record: `master/relays/c4-audit-m-7/AUDIT-planner-<ts>.md`.)*
