...
S1 renders `grant` only on operator/orchestrator forms. Conditional pair-Planner grant rendering lands in S3 through the form-system registry for fresh stores: the S3 registry rides `store.Init`; registry/config evolution on an existing store is live through the operator-authorized §7 record, effective at restart, and this S1 schema keeps that later landing additive rather than foreclosed.
