...
2. OPERATOR — storage backing + markdown role: typed canonical storage as JSON, SQLite row/object, or both; markdown as a rendered view only vs a signed export; and how strict v3.0 is about rejecting legacy hand-authored markdown relays.
...
- Produce the dissolve/survive TABLE where EVERY dissolved v2.8.8 relay-lint check is classified as one of: prose-only (genuinely no semantics), typed-form-validation (survives at fill-time), or cross-relay-lineage (survives in the separate engine). No check may be dropped without a class.
