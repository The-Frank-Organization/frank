...
IN_REPLY_TO: master/relays/s6-amend-m-2/DESIGN-planner-20260706-184500.md
...
3. **F12's header fallback is under-specified for an operator-scoped arbitrary rationale.**
   ...
   The amendment correctly rejects the current `ORCH_REVIEW_WAIVER: *` presence-only shape, but the fallback "header carries free_text rationale" branch is not yet a complete m-2 schema contract. The current FieldSpec model has a seat-scoped enum path for constrained values (`frank/internal/fieldspec/registry.json:134`), and generic field type/options metadata (`frank/internal/fieldspec/fieldspec.go:15-20`); it does not by itself express "operator-scoped arbitrary free_text header value" with submit-time enforcement. m-1's F17 direction is an operator-only waiver record class with scope/retraction semantics (`master/domains/m-1-trust-identity/design/2026-07-06-s6-transport-amendments.md:43-51`).
...
RELAY_LINT: OK - relay-lint.py exit 0 on `master/relays/s6-amend-m-2/DESIGN-REVIEW-implementer-20260706-184218.md`
