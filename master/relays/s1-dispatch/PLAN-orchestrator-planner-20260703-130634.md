...
- **Spec = read-only reference in cwd:** `master/ARCHITECTURE.md` §C4 (the engine) + the m-1 / m-2 / m-7 domain docs. **Do not edit governance docs**; escalate spec problems to master — do not self-amend a locked design.
...
- a **minimal dissolved-linter replay** over the historical v2.8.8 failures the MVP FieldSpec covers.
...
- **Dissolved-linter replay (S1-minimal):** the MVP-covered historical v2.8.8 failures run through the MVP validator are caught-or-genuinely-obsolete. (The full ~33-check replay is an S3 gate.)
...
- Pointers: `master/ARCHITECTURE.md` §C4 + §C4.3/I-PH, `master/STEP-1-KICKOFF.md`, `ROADMAP.md` Step-1, `frank/` (empty repo, branch `main`, 0 commits).
