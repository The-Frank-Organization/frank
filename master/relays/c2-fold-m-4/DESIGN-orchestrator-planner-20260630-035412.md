...
**F2 — record a per-assignment home for `seat_archetype` + resolved `authority_ceiling` (the non-template path).** Your `routing_assignments` row has no archetype/ceiling column. For TEMPLATE-spawned teams `template_ref` carries the per-seat archetype; but for HAND-AUTHORED (non-template) staffing — e.g. a planner directly staffing a sensor seat — the ceiling that made write authority unavailable must still be **recorded for replay/audit**. **VP-approved (per-assignment field preferred):** add an **opaque** per-assignment `seat_archetype` (and/or resolved `authority_ceiling`) column to `routing_assignments` for replay/provenance; OR explicitly require all v3.0 archetype-bearing spawns to go through template records. Pick one; the per-assignment field is the replay-complete option both m-5 seats preferred.
