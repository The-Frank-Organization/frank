...
| 3-staged policy (prior → justified deviation → [v3.1] outcome feedback) | **agree** | identical staging; benchmark loop = forward hook, not Step-0. |
...
| Cost/latency/privacy `constraints` | **different-coverage → fold as optional/forward** | capability-prior is the v3.0 floor; constraints are optional policy inputs / reserved (the external cost-quality literature lives here). Settle weight at DESIGN. |
...
- **(b)** Non-gradient bandit outcome-feedback already exists (C2MAB-V 2405.16587; PILOT/LinUCB 2508.21141) — so the v3.1 loop's novelty is the *auditable, persisted decision+update artifact*, **not** the non-gradient mechanism. The "Fugu-reward-analog" framing must be hedged accordingly.
...
2. **v3.0 record scope** — recommend recording the decision for human-launched lanes now (Step-1 "automated operator-relay"), execution deferred to Step-3. Confirm at PROCEED-TO-DESIGN.
