...
Domain context. m-4 owns **routing & policy** — the conductor's thesis. The model→seat router, capability priors, the routing record, justified deviation, and the benchmark + v3.1 feedback loop. Locked high-level decisions you build on: altitude **B** (the planner emits role+model per dispatch), policy **3-staged** (capability priors + justified deviation; benchmark feedback arrives in v3.1). The differentiator: routing is a first-class, recorded, *justifiable* governance decision — not implicit config.
...
- Export design intent — **READ FIRST**: `extracted/agentic-dev-team-skills-v3-export/v3-design/v3-adaptive-routing-pillar.md` (the m-4 spec seed: altitude B, capability priors, the 3-staged policy, justified deviation, the benchmark/v3.1 loop) and `EXTERNAL-REFERENCES.md`.
- v2.8.8 current state: `extracted/agentic-dev-team-skills-v3-export/v2.8.8-release/v288-unzipped/agentic-dev-team-skills-v2.8.8/` — what routing/model-selection (if any) v2.8.8 has; expect this to be a GAP (single-model relay) and to confirm the still-open verdict. Our LIVE store at `master/relays/` shows hand-routing (role+model chosen per dispatch by the operator).
...
3. A design recommendation: the router API surface, the capability-prior representation, the routing-record schema (as a consumer of the locked m-2 FieldSpec/R2 shape), and the 3-staged policy mechanism — with the benchmark/v3.1 loop as a forward hook, not Step-0 closure.
...
- m-3↔m-4 SEAM: your benchmark/v3.1 loop consumes m-3 observed evidence; a routing record may be an evidenced record. Flag seam findings; a dedicated `c2-*-coord` COORD sub-thread owns the seam at DESIGN (reconciled before any c2 lock).
...
- Reads: the export adaptive-routing pillar; v2.8.8 routing gap; jcode MultiProvider + claude-code/agent-scripts model-selection; the locked c1 R2/§5/§J seam.
- Target entity: the v3 routing/policy primitive (a design recommendation, not code).
