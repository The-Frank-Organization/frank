...
- **already-closed (promote/wire):** ODB content schema (agent-scripts `:53-69` + m-2 ODB slots `:260-276`); A/B/C/D bucket drivers (m-2 `:250-258` + §J `:89-102`); m-1 operator mailbox/projection target (`m-1 …design.md:107-118`); m-3 egress + observe + `record_integrity` (`:41-63,116-122,146-151`); m-4 routing escalation, no new gate class (`:144-158,211-213`); v2.8.8 TO/CC semantics (`protocol.md:28-40`); decision-ready + freshness discipline (agent-scripts `:41-51,57,69`).
...
- **OJ-6** delegation/backup-approver: recommend DEFER for v3.0 (solo-operator); if ever added, a stamped auditable relay, never silent substitution.
...
- E1 sources: as cited in the two passes (`ARCHITECTURE.md` §J/C2; m-1/m-2/m-3/m-4 design docs; agent-scripts; v2.8.8; `v3-adaptive-routing-pillar.md`; jcode/codex look-notes) + the three research records (R1 v2.8.8 / R2 agent-scripts+export / R3 external prior-art).
