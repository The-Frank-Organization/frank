...
Domain context. m-3 owns **observation & evidence**: the observe-as-send-gate (the conductor reads lane ground-truth from *outside* the lane before a relay is deliverable), per-phase done-predicates, the evidence ladder (E-tiers), executable claims, and the fail-closed egress/content-safety gate (secrets / PII / model-names). The v3 stance: a "done" is only as good as conductor-observed evidence; self-reported completion is *labeled*, never trusted.
...
- v2.8.8 current state: `extracted/agentic-dev-team-skills-v3-export/v2.8.8-release/v288-unzipped/agentic-dev-team-skills-v2.8.8/` — protocol.md's evidence/observe model, done-predicates, the orchestrator-review/observe gates in `tools/relay-lint.py`, and any "blocks-before-dispatch" evidence handling. Our LIVE store at `master/relays/` is a running instance.
...
- Export design intent: `extracted/agentic-dev-team-skills-v3-export/v3-design/v3-adaptive-routing-pillar.md` (observe/evidence + the conductor's gate components) and `EXTERNAL-REFERENCES.md`.
...
1. A 4-bucket verdict (still-open / already-closed / product-overlapped / recommended-next) on the v3 m-3 observe/evidence primitive vs what v2.8.8 and the references already provide.
...
4. A boundary contract naming what m-3 must expose to consumers — m-4 (observed evidence feeding the benchmark/v3.1 routing-quality loop), m-5 (per-archetype observe invariants), m-6 (gate mechanism + egress for the away-mode external bridge) — so DESIGN locks interfaces consumer-validated.
...
- m-3↔m-4 SEAM: m-4's benchmark/v3.1 loop consumes your observed evidence; a routing record may be an evidenced record. Flag seam findings; a dedicated `c2-*-coord` COORD sub-thread will own the seam at DESIGN (reconciled before any c2 lock). Lighter than c1's PARENT/lineage convergence.
...
- Reads: v2.8.8 evidence/observe handling; jcode/claude-code/agent-scripts gating + egress prior art; the export design intent; the locked c1 R3/DI-5 seam.
- Target entity: the v3 observe/evidence primitive (a design recommendation, not code).
...
0. Duplicate/already-built check: does v2.8.8 or a reference already provide an observe-gate / egress scan we should promote rather than rebuild? If so, recommend promote/wire, do not rebuild.
