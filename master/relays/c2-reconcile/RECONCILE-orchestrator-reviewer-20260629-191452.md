...
4. The F5 qualifications are material acceptance criteria. The m-4 design must say that v3's novelty is a seat-stamped, persisted, auditable routing/deviation artifact, not interpretable routing in general or non-gradient adaptation in general. The m-3 design must likewise avoid overstating novelty where v2.8.8, jcode, claude-code, and agent-scripts already provide partial hook/evidence/egress primitives.
