...
**Operator-judgment items (carried forward; none blocking AUDIT→DESIGN).** m-3: executable-claim execution surface (registry-approved descriptors vs arbitrary commands; reserved seam, v3.0 ships without); egress fail-closed policy (auto-redact-low-risk vs always-block-on-first-release; config); Step-1 read-vantage honest-fallback floor. m-4: capability-prior seed values (ship a default seed, operator confirms/customizes); v3.0 record scope = human-launched lanes now, execution Step-3. Most mirror the already-ratified §J config-customizability pattern. Recommend they ride to DESIGN as operator items, decided at/with the c2 design-lock — not now.
