...
Load **agent-pair-implementer** (+ `protocol.md` v2.8.8).
...
**Audit/review corpus:** v2.8.8 runtime; jcode/claude-code process + attach prior-art; the runtime research incl. the primary-source-verified `srt` + Codex app-server facts (`master/RUNTIME-RESEARCH.md`, esp. §8 + :772-780); crash-atomicity / serialized-commit prior-art; the `DESIGN-REVIEW` §2A findings; the locked m-1..m-6 domain design docs as **contract inputs**.
