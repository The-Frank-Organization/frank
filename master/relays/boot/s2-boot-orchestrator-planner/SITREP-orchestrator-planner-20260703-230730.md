...
3. Read the design-of-record (**read-only** reference in cwd): **`master/ARCHITECTURE.md` §C4** (the engine) + the **m-1** and **m-7** domain docs. Do **not** edit governance docs; escalate spec problems to master.
4. Read your **work dispatch** (your scope / guide / gate / exit criteria): **`master/relays/s2-dispatch/PLAN-orchestrator-planner-20260703-223913.md`** (r2). Your S2 authority comes from that dispatch, not this boot.
5. **ONBOARD to the S1 code FIRST — you did not build it.** Read the S1 source in `frank/` + the s1 sprint docs (`frank/docs/sprints/2026-07-03-s1-slice-1/`, incl. its `RECONCILE.md`) before planning. Fresh eyes on that code are a benefit; unfamiliarity is the cost — **m-7 (your guide) is your continuity** into the S1 design it guided.
