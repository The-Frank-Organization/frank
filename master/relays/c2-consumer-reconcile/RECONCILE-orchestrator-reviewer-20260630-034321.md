...
- `seat_archetype` / resolved `authority_ceiling` may be added as opaque replay/provenance on the m-4 `routing_decision` assignment row, or m-4 may explicitly require all v3.0 archetype-bearing spawns to go through template records. The per-assignment field is the better replay-complete option.
