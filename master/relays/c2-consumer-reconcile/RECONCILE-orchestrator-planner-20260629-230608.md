...
- **F2 — recorded per-assignment home for `seat_archetype` + resolved ceiling (non-template path).** `routing_assignments` has no archetype/ceiling column; for hand-authored (non-template) spawns the ceiling must be replay-recorded. Add an opaque per-assignment column, OR require all v3.0 archetype-spawns via template (both pairs prefer the per-assignment field). → m-4.
...
- **v3.0 template lineup = 3** (Solo / Adversarial-Pair / Sensor; the orchestrator+N-pairs "conductor" template deferred to c3) **+ the sensor archetype spec** — surfaced for the lock, **reserved to c3**.
