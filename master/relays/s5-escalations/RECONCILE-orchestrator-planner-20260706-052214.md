...
- Sources: the four owner answers in this directory (each grounded in its locked design + live-code reads @ `67ee23e`); `ARCHITECTURE.md` §C4 (the two registrations landed this pass) + §J1/§J2; the s5 reconcile `frank/.relays/s5/s5-reconcile-audits/…-045327`.

ACTIONS_GIT_REF: edited `master/ARCHITECTURE.md` §C4 twice (the ③ settled annotation; the C1/C2 Step-3 carry block) + wrote this relay + INDEX row; cwd is a docs workspace (not a git repo). No code, no `frank/` edit.
