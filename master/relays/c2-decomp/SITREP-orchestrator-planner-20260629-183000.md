...
2. **m-4 is the headline.** The adaptive-routing pillar (altitude B, capability priors, 3-staged policy + justified deviation, the benchmark/v3.1 loop) is the conductor's thesis. Its *record contract* is already half-specified — R2 in `ARCHITECTURE.md` (routing = a separate seat-stamped relay; model never a gate input). c2 designs the full router/policy on top of R2.
...
- Q3 — audit depth: full fresh domain audit (v2.8.8 + jcode/claude-code/agent-scripts + the v3 adaptive-routing-pillar export), or a compressed audit that leverages the c1 consumer-review the pairs already did? I lean full-but-focused (the consumer-review audited the *foundation*, not their own domain's prior art).
