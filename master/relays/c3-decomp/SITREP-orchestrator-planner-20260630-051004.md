...
- **Q3 — audit depth.** Full fresh domain audit, or compressed leveraging m-5's c2 narrow engagement + m-6's c1/c2 consumer-lenses? I lean **full-but-focused**: m-6 gets a full domain prior-art audit (agent-scripts Owner Decision Brief, v2.8.8 email-governance/meeting surfaces, jcode/claude-code human-surface, scheduler park/wake — never audited as its own domain); m-5 gets a focused audit (it reviewed the seam in c2; now it audits archetype/workflow prior-art — v2.8.8 expansion-slots, codex `collaboration-mode-templates` + `agent-graph-store`, claude-code subagents/`sideQuestion`, the v3 export — and binds its reserved proposals).
