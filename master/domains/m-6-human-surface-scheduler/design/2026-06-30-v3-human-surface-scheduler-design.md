# v3 m-6 — Human Surface & Scheduler (DESIGN-OF-RECORD)
...
**The organizing frame — three human-surface lanes** (the export's governance-vs-collaboration split, `v3-adaptive-routing-pillar.md:51`):
...
The inbox is a **projection of the locked addressing graph** (`v3-adaptive-routing-pillar.md:37`), not a new store. Four buckets, each with a **locked writer** (no-bucket-without-a-writer); **direction-explicit** — only **A + C** reach the operator.
...
- The bounded choices render as **buttons** (one per enumerated choice). The choice-set is a **self-describing, capability-scoped gate** realized as m-2 **`agent_enum_pick`** (`v3-adaptive-routing-pillar.md:49`) — the operator can pick **only a legal verdict**.
...
- **`away_bridge_eligible` = an m-6-owned per-gate boolean**, defaulted by the locked policy (A-bucket ∧ opt-in ∧ egress-clear); m-5 declares only the `away` posture (seam-converged `123022`⊕`131856`). A hard per-archetype never-bridge ceiling is a **reserved future m-5 hook** (§10 OQ-3), not needed for v3.0.
...
- **OQ-3 [operator-declined / reserved] — write-capable elaboration fork is DECLINED** (operator 2026-06-30: the elaboration fork stays read-only; its only output is the decision relayed back). A hard per-archetype **never-bridge ceiling** (beyond content-egress) remains a reserved future hook — not needed for v3.0.
