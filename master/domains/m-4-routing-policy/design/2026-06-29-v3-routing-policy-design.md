# v3 Routing & Policy — design (the governance-record layer)
...
> Every surveyed system selects a model with no recorded, justifiable, auditable decision; v3 ports the
...
- **GL-2 — v3.1 tuning scope = recommendations only.** The v3.1 outcome-feedback loop may retune **Layer 2
...
- **GL-3 — v3.0 record scope = record-now / execute-Step-3 (option B).** v3.0 records the routing decision
  while **riding existing runtimes**; the conductor does not drive its own provider adapters (that is Step-3 per
  `ROADMAP.md`). v3.0 enforces **declaration honesty** (the deviation check, §2), not **execution fidelity**
...
- **GL-4 — v3.0 ships routing TEMPLATES (operator-directed scope addition).** v3.0 ships 1–3 selectable team
...
(*which* model ran), never a governed *why* a human can audit and override before it takes effect. v2.8.8: no
...
which under "model = payload, not trust-bearing" v3.0 explicitly does **not** require. The honesty of the
...
- **Step-1 vs Step-3:** in v3.0 the record is *written* and the human (or the GL-4 template-spawn) launches the
...
Layer 2 — bucket recommendation ((role, task_tag) → ranked buckets; v3.1-tunable per GL-2):
...
- **Declared, not learned** — seeded from Fugu's published priors (GPT→math, Gemini→science/GPQA, Opus→SWE/
  ...
  row carries a **`basis`** (benchmark name / model-card claim / [v3.1] observed-outcome) — what makes it a
...
- **Snapshotted into the record** (`capability_prior_snapshot`, system-filled `computed_result`,
  ...
  cannot be reconstructed after the fact, and v3.1 must attribute each outcome to the prior version in force.
...
value set — `routing_decision`; reserved `routing_deviation` / `routing_update` for v3.1 — reusing the
...
## §6. The 3-staged policy + the v3.1 forward hook
...
- **Stage 2 — justified deviation (ships v3.0).** The planner may pick off-floor → declares
...
- **Stage 3 — outcome feedback (v3.1 forward hook only).** The benchmark consumes m-3 observed outcomes (via
  ...
  gate. **v3.1 sampling caveat (held):** draw the outcome sample **independent of `declared_deviated`** — a
...
- **`constraints` (budget/latency/privacy)** are **reserved/forward** policy inputs (the external cost-quality
  literature lives here); capability-prior is the v3.0 floor. Their weight in the policy is an open question
  for a later cycle (§13), not a v3.0 closure.
...
opening/naming the panes + boot-relay delivery. Roadmap sequencing of the v3.0 scope addition is the
...
> ratification at the c2 lock. Task-outcome evidence (`outcome_feedback_ref`) is null-reserved in v3.0 and
> benchmark-sampled in v3.1 (sampled independent of `deviated`).
...
- **→ m-3:** the `routing_decision` record is a possible **evidenced record**; the benchmark/v3.1 loop consumes
...
runtime + provider adapters + the router executing the Step-1 record + the benchmark). v3.0 (Step-1) **rides
existing runtimes** and does not drive its own agents.

**Documented boundary (the way v2.8.5 §10 documented deferred forgery):** v3.0 enforces **declaration honesty**
(§2, all lanes, snapshot-provenance) but **not execution fidelity** (whether the launched lane actually ran the
recorded model). **This execution-fidelity gap is a documented v3.0 residual (D5-class): a lane may serve a
different model than the record declares, and v3.0 does not verify it** — verifying the serving model would need
outside model-attribution (m-1 channel territory), explicitly out of scope under "model = payload." **At Step-3
(future — standalone-runtime hardening, NOT a v3.0/Step-1 claim)** the record *becomes* the spawn call, so the
...
GL-4 template-spawn already narrows the gap in practice for v3.0 (each session opens with its preset model).
...
1. **`constraints` (budget/latency/privacy) weight in the policy** — reserved for a later cycle; v3.0 is
...
FieldSpec `router`/`routing_ref` — `master/domains/m-2-forms-determinism/design/2026-06-28-v3-form-schema-design.md:60-63`;
pillar altitude-B/3-staged/Fugu-priors — `extracted/agentic-dev-team-skills-v3-export/v3-design/v3-adaptive-routing-pillar.md:17-18,68`;
...
- **SCOPE (1)** — **§12 :381–:387** the execution-fidelity gap is now named a **documented v3.0 residual (D5-class)**;
  "Step-3 closes the gap **structurally**" is explicitly scoped to **Step-3 only (future standalone-runtime
  hardening, NOT a v3.0/Step-1 claim)**.
