...
**Cycle c2 scope:** AUDIT + DESIGN of the v3 routing/policy domain (the adaptive-routing pillar), against the
...
**benchmark + v3.1 feedback loop**. Locked high-level decisions this builds on: altitude **B** (planner emits
role+model per dispatch), policy **3-staged** (capability priors + justified deviation; benchmark feedback in
v3.1). The differentiator: routing is a first-class, recorded, justifiable governance decision — not an
implicit config.
...
- **Co-design seam with m-3 (this cycle):** the benchmark / v3.1 loop **consumes m-3 observed evidence** for
...
- AUDIT: **reconciled** `c2-audit-m-4` (20260629). Independent planner + implementer passes + RECONCILE filed
  ...
  audit artifact); non-gradient bandit feedback already exists (v3.1 novelty = the auditable persisted
...
- DESIGN: **COMPLETE / held for c2 lock** `c2-design-m-4` (20260629). DESIGN_DOC_ID
  `c2-design-m-4-routing-policy` (`design/2026-06-29-v3-routing-policy-design.md`), operator-approved +
  **m-4.implementer DESIGN-REVIEW approve** on rev1 (`c2-design-m-4/DESIGN-REVIEW-implementer-20260629-203329.md`;
  ...
  (`c2-design-m3-m4-coord`). Operator grill-locks (§0): GL-1 two-layer bucket priors; GL-2 v3.1 tunes
  recommendations only; GL-3 record-now/execute-Step-3; GL-4 v3.0 ships 1–3 routing templates (m-4 owns
...
- CQ-CLOSURE c4-cq-gateconfig (m-7 conductor-core gate CQs; m-4 owns **CQ-4b only** — trusted-config composition):
  **planner-lead confirm produced, in full-pair review** (20260702). m-4 was re-engaged (VP added m-4 to the CQ-4b
  ...
  (`c4-cq-gateconfig/DESIGN-planner-20260702-013000.md`): **preserves v3.0 cleanly + beneficial** (single digest = a
  ...
  stamp for the Layer-2 recommendation table** so v3.1 auto-tuning (GL-2; §6 Stage-3 "bolts on without re-cutting the
  record/gate") isn't precluded by "operator-authorized full-artifact recompute." Layer-1 stays top-level-digest-bound.
  CQ-4b (m-4) → **corrected-by-artifact**; v3.1 auto-tune cadence → non-locking-carry; CQ-2/3/4 not m-4's. Routed to
  ...
  **m-4.implementer returned `must-revise`** (`c4-cq-gateconfig/DESIGN-REVIEW-implementer-20260702-020443.md`) — a
  ...
  (`c4-cq-gateconfig/DESIGN-planner-20260702-021000.md`): v3.0 confirm unchanged; Layer-2 ask narrowed to an
  **optional per-section `version` reservation inside the single top-level digest** (no v3.0 semantics; every effective
  v3.0 change still recomputes the top-level digest + operator-authorized reload); v3.1 auto-tune cadence/authorization
  = **still-open/non-locking-carry** (new design if v3.1 pursued). CQ-4b (m-4) → corrected-by-artifact, co-signable.
...
- CLAIM-SWEEP c5-claim-sweep-light (claim-text hygiene, batched m-3/m-4/m-5/m-6): **m-4 folded, → implementer approve**
  ...
  (rides m-1 stamp; mirror m-1). **1 SCOPE** — §12 execution-fidelity gap named a documented v3.0 residual (D5-class);
...
Under the reframe, **routing EXECUTION defers to Step-4**. Step-3 = the one-governed-turn MVP with a **single pinned app-side run manifest** (m-10-owned run state: immutable m-8 lane ID + catalog digest) — **NOT an m-4 governed routing decision, and NO lane-bearing conductor FieldSpec row**. The m-4/m-2 routing-record amendment (`step3-amend-m4-routing`) is **DEFERRED to the named Step-4 routing-execution gate**; m-4 may **consumer-review** the manifest boundary now but does **NOT re-dispatch as a Step-3 writer**. The **`GL-3 record-now/execute-Step-3`** line and any in-Step-3 routing-execution framing are **SUPERSEDED for Step-3** (retained as record). When the routing DECISION record lands (Step-4) it stays a **conductor relay governed by m-4 + shaped by m-2 with R2 non-gate-referenceability**; the executable run BINDING is app-side. The old sections above are retained as record, **not operative for Step-3**.
