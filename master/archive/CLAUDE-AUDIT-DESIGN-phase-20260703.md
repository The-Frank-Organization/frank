...
1. **Phase scope = AUDIT + DESIGN only.** The team audits the current state and designs the frank
   architecture-of-record. No PLAN / IMPL / MERGE this phase. We terminate at per-domain **design-lock**.
2. **Docs live in cwd, never in `pcode/`.** `pcode/` is the (empty) target git repo for *future* frank code;
   do not write docs there or implement there this phase.
...
4. **Relay discipline.** Every substantive handoff is a lint-clean file relay under `master/relays/`. The
   ...
   orchestrator relays (AUDIT, DESIGN, REVIEW-FOLD, …) CC `master.orchestrator-reviewer`.

## The product we're governing
**"the conductor":** our own thin trusted courier (NOT built on jcode / claude-code — prior-art
references only). Locked high-level decisions — altitude **B** (planner emits role+model per dispatch),
